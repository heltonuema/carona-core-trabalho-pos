...
//		incluiUsuario("andre", "andre@murta.com");
//		oferecerCarona("andre", "andre@murta.com", "Aguas Claras", new Horario(9,0), "Iesb", "Asa Norte");
//		
//		incluiUsuario("helton", "helton@uema.com");
		
	}
	
	/**
	 * Faz o cadastro de um usuário novo no servidor e retorna a mensagem de erro ou sucesso
	...
	public String cadastrarUsuario() throws ClientProtocolException, IOException{
		...
		HttpEntity entity = new StringEntity(usuario.toString(), "UTF-8");
	 ...
	 * @throws ClientProtocolException
	 * @throws IOException
	 * @throws RuntimeException se o servidor retornar algum status diferente de 200
	 */
	
	public Carona oferecerCarona(final String pontoPartida, final Horario horario,
			final String localDestino, final String bairroDestino, final int maximoPassageiros) throws ClientProtocolException, IOException{
		...
		if(response.getStatusLine().getStatusCode() != HttpStatus.SC_OK){
			throw new RuntimeException("Carona não incluída: " + CaronaClient.getStringFromInputStream(response.getEntity().getContent()));
		}
		
		return new Gson().fromJson(CaronaClient
				.getStringFromInputStream(response.getEntity().getContent()), Carona.class);
	}
	
	/**
	 * Faz solicitação para inclusão do usuário em uma carona e retorna a mensagem devolvida pelo servidor
	 * @param idCarona
	 * @return
	 * @throws ClientProtocolException
	 * @throws IOException
	...
	public String solicitarParticipacaoCarona(final String idCarona) throws ClientProtocolException, IOException{
		...
		HttpClient client = new DefaultHttpClient();
		HttpPost post = new HttpPost();
		URI uri = URI.create(serverAddress + "/carona-iesb/api/caronas/solicitarParticipacao/"+idCarona+"/"+usuario.getEmail());
		post.setURI(uri);
		HttpResponse response = client.execute(post);
	 ...
	 * @throws IOException
	 */
	public String despacharSolicitacaoCarona(final long idCaronaPendente, final boolean aprovar) throws ClientProtocolException, IOException{
		
		HttpClient client = new DefaultHttpClient();
		HttpPost post = new HttpPost();
		URI uri = URI.create(serverAddress + "/despacharSolicitacao/"+String.valueOf(idCaronaPendente)+"/"+String.valueOf(aprovar));
		post.setURI(uri);
		HttpResponse response = client.execute(post);
	...
	public List<Carona>consultaCarona(final String partida, final String localDestino, final String bairroDestino) throws ClientProtocolException, IOException{
		
		String queryParam = new String();
		if(!isNullOrEmpty(partida)){
			queryParam = queryParam.concat("partida="+partida);
		}
		if(!isNullOrEmpty(localDestino)){
			if(!queryParam.isEmpty()){
				queryParam = queryParam.concat("&");
			}
			queryParam = queryParam.concat("localDestino="+localDestino);
		}
		if(!isNullOrEmpty(bairroDestino)){
			if(!queryParam.isEmpty()){
				queryParam = queryParam.concat("&");
			}
			queryParam = queryParam.concat("bairroDestino="+bairroDestino);
		}
		...
		Type type = new TypeToken<List<Carona>>(){}.getType();
		List<Carona> retorno = gson.fromJson(caronaJson,type);
		
		return retorno;
	}
	
	
	private HttpResponse executaRequisicao(final HttpRequestBase method, final URI uri) throws ClientProtocolException, IOException{
		HttpClient client = new DefaultHttpClient();
		method.setURI(uri);
